...
class RegisterRequest(BaseModel):
    ...
    professional_title: str | None = Field(None, description="职称（教师必填）")
    research_direction: str | None = Field(None, description="专业方向（教师必填）")
    teacher_no: str | None = Field(None, description="教师工号（教师必填）")
    student_no: str | None = Field(None, description="学号（学生必填）")
    major: str | None = Field(None, description="专业（学生必填）")
    ...
    def check_role_fields(self) -> "RegisterRequest":
        ...
        - 教师：必须提供工号 (teacher_no)、学院 (college)、职称 (professional_title) 和专业方向 (research_direction)。
        - 学生：必须提供学号 (student_no)、导师工号 (advisor_no)、学院 (college) 和专业 (major)。
        ...
        if self.role == BorrowerType.TEACHER:
            ...
            if not self.professional_title:
                raise ValueError("professional_title is required for teacher")
            if not self.research_direction:
                raise ValueError("research_direction is required for teacher")
